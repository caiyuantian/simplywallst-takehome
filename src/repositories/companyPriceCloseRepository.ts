...
export class CompanyPriceCloseRepository {
  getLatestCompanyPriceClose = () => {
    return AppDataSource.manager.query(
      `SELECT a.* from swsCompanyPriceClose a
  ...
  getCompanyVolatility = (days: number) => {
    return AppDataSource.manager.query(
      `SELECT T1.company_id, ROUND(SQRT(AVG((T1.price - T2.average) * (T1.price - T2.average))),2) as volatility from swsCompanyPriceClose T1
        INNER join(
